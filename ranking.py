...
from preprocess import preprocess_id, preprocess_context,strip_html
...
def get_ranking(triplets, minimum_freq = 1):
    ...
    for (text,ids0,ids1) in triplets:
        ...
        # And preprocess the ids
        ids0_n = []
        for ids in ids0:
            ids0_n.append(preprocess_id(ids))

        ids1_n = []
        for ids in ids1:
            ids1_n.append(preprocess_id(ids))

        # Let's now get all contexts and building a list with it.
        for t in extract_all_contexts(ids0_n,ids1_n,text,both_ways=True):
